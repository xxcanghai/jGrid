...
module jch {
    export interface jCheckBoxSetting {
        element?: JQuery;
    ...
    export class jCheckBox {
        public static workModeEnum = {
            //jCheckBox工作模式
            normal: "normal",//复选框（单独的）
            checkBox: "checkBox",//复选框（被全选管理中的）
            checkall: "checkall",//复选框管理者——全选框
            radioButton: "radioButton",//单选钮（被单选组管理中的）
            radioButtonGroup: "radioButtonGroup"//单选钮管理者（隐藏）
        };
        public static styleModeEnum = {
            //jCheckBox的显示样式
        ...
        public static enabledState = {
            //复选框启用禁用状态
            disabled: 0,//禁用
            enabled: 1//启用
        };
        public static attrEnum = {
            jchecked: "jchecked",
            jenabled: "jenabled",
            styleMode: "stylemode",
            workMode: "workmode"
        ...
        public config: jCheckBoxSetting = null;
        public element: JQuery = null;
        public subCheckBoxArr: jCheckBox[] = [];

        private workModeEnum = jch.jCheckBox.workModeEnum;
        private styleModeEnum = jch.jCheckBox.styleModeEnum;
        private checkedListener: ((targetJcb: jCheckBox, e: Event, chkState: boolean) => void) = $.noop;
        private workMode: string = jCheckBox.workModeEnum.normal;
        private styleMode: string = jCheckBox.styleModeEnum.checkBox;

        public constructor(cfg: jCheckBoxSetting) {
            if (!(this instanceof jch.jCheckBox)) return new jch.jCheckBox(cfg);
            //!+ ***jCheckBox默认选项***
            var dft: jCheckBoxSetting = {
                element: $("<span>"), //插件对应jquery对象
                checked: false, //是否选中.布尔值或函数，函数无参数，返回布尔值来设定是否选中
                enabled: true, //是否可用.布尔值或函数，函数无参数，返回布尔值来设定是否可用
                //--Event--
                //所有事件内this指向为jCheckBox
                onCheckedChange: $.noop, //当checked属性变更后发生,第1个参数 chkState为已经变更的布尔值选中状态,第2个参数为事件对象 
                onBeforeCheckedChange: $.noop, //当checked属性变更前发生，第1个参数  chkState为即将变更为的布尔值选中状态,第2个参数为事件对象
                onEnabledChange: $.noop, //当enabled属性变更后发生,参数1个 enbState为已经变更的布尔值可用状态
        ...
        private createDom(): void {
            var c: jCheckBoxSetting = this.config;
            var enabled: string = ((typeof (c.enabled) === "function") ?
                !!((<() => boolean>c.enabled).call(this)) :
                !!c.enabled).toString();
            var checked: string = ((typeof (c.checked) === "function") ?
                !!((<() => boolean>c.checked).call(this)) :
                !!c.checked).toString();
            var attrEm = jch.jCheckBox.attrEnum;
            this.element.addClass("jCheckBox")
                .attr(attrEm.jenabled, enabled)
                .attr(attrEm.jchecked, checked)
                .attr(attrEm.workMode, this.workMode)
                .attr(attrEm.styleMode, this.styleMode)
                .data("jCheckBox", this);
        }
         ...
         * @returns
         */
        private checkFunctionBind(): void {
         ...
         * @returns
         */
        private eventBind(fn: Function, thisobj: Object): any {
         ...
         * @returns
         */
        private checkBoxClick(e: Event, me: jCheckBox): void {
            if (me.getIsEnabled() == false) return;
         ...
         * @returns
         */
        private checkBoxSelectStart(e: Event, me: jCheckBox): void {
            e.preventDefault();
        }
         ...
         * @returns
         */
        private getBoolByAttr(attrName: string): boolean {
            var str = this.element.attr(attrName).toLowerCase();
            var bool: boolean|any = (str == true.toString()) ? true : (str == false.toString()) ? false : null;
         ...
         * @returns
         */
        public change(chkState?: boolean, isCheckAllTrigger: boolean = false, e?: Event): void {
            if (chkState === undefined) chkState = !this.getIsChecked();
            chkState = !!chkState;
            if (chkState === this.getIsChecked()) { return; }
            if (this.workMode == this.workModeEnum.radioButton && chkState == false && !isCheckAllTrigger) return;//单选钮不允许取消选择
            if (this.workMode == this.workModeEnum.radioButtonGroup) { throw Error("radioButtonGroup action error!"); return; }//单选钮组模式，不允许操作
            if (this.config.onBeforeCheckedChange.call(this, chkState, e) === false) { return; }
            ...
            if (!isCheckAllTrigger) {
                ...
                if (this.workMode == this.workModeEnum.checkall) {
                    ...
                    $.each(this.getSubCheckBoxArr(true), function (i: number, a: jCheckBox) {
                        if ((a instanceof jch.jCheckBox) === false) return true;
         ...
         * @returns
         */
        public enabled(enbState: boolean = true): void {
            enbState = !!enbState;
            if (enbState === this.getIsEnabled()) return;
         ...
         * @returns
         */
        public setWorkMode(wm: string): void {
            if (this.workMode != this.workModeEnum.checkall) {//当复选框同时工作在普通复选框和全选模式下，不予修改（例如：层级树状复选框）
                this.workMode = wm;
            }
         ...
         * @returns
         */
        public setStyleMode(sm: string): void {
            this.styleMode = sm;
         ...
         * @returns
         */
        private getStyleModeByWorkMode(wm: string): string {
            var sm: string = "";
         ...
         * @returns
         */
        public setCheckedListener(fn: (targetJcb: jCheckBox, e: Event, chkState: boolean) => void): void {
         ...
         * @returns
         */
        public initCheckAllMode(jcbarr: jCheckBox[]): void {
            this.setWorkMode(this.workModeEnum.checkall);
            this.subCheckBoxArr = $.extend(true, [], jcbarr);
            var me = this;
            $.each(this.subCheckBoxArr, function (i: number, a: jCheckBox) {
                if ((a instanceof jch.jCheckBox) === false) return true;
         ...
         * @returns
         */
        public initRadioButtonGroupMode(jcbarr: jCheckBox[]): void {
         ...
         * @returns
         */
        public inspectCheckAll(targetJcb: jCheckBox, e: Event, chkState: boolean): void {
            var subCbNum: number = this.getSubCheckBoxArr(true).length;//所有可用项
            var checkedNum: number = this.getSubCheckBoxArr(true, true).length;//可用的勾选项
            var uncheckedNum: number = this.getSubCheckBoxArr(true, false).length;//可用的没选项
            if (subCbNum == 0) {
                //下属没有复选框，什么都不做
                return;
         ...
         * @returns
         */
        private inspectRadioButtonGroup(targetJcb: jCheckBox, e: Event, chkState: boolean): void {
            //*注意：此函数需要在[单选钮组]对象上执行
            var subCb: jCheckBox[] = this.getSubCheckBoxArr(true);//所有可用项
            $.each(subCb, function (i: number, cb: jCheckBox) {
                if (cb !== targetJcb) {
                    //console.log(cb.element, targetJcb.element, false);
                    cb.change(false, true);
                } else {
                    //console.log(cb.element, targetJcb.element, "Select!");
                }
            });
        }
         ...
         * @returns
         */
        public getSubCheckBoxArr(isEnabled?: boolean, isChecked?: boolean) {
            //过滤子对象中的非jcb对象
            this.subCheckBoxArr = $.grep(this.subCheckBoxArr, function (a: jCheckBox, i: number) { return a instanceof jch.jCheckBox; });
            if (!isBool(isEnabled) && !isBool(isChecked)) {
                return this.subCheckBoxArr;
            } else if (isBool(isEnabled) && !isBool(isChecked)) {
                return $.grep(this.subCheckBoxArr, function (a, i) { return a.getIsEnabled() === isEnabled; });
            } else if (!isBool(isEnabled) && isBool(isChecked)) {
                return $.grep(this.subCheckBoxArr, function (a, i) { return a.getIsChecked() === isChecked; });
            } else if (isBool(isEnabled) && isBool(isChecked)) {
                return $.grep(this.subCheckBoxArr, function (a, i) { return a.getIsChecked() === isChecked && a.getIsEnabled() === isEnabled; });
            } else {
                return this.subCheckBoxArr;
            }

            function isBool(arg: boolean): boolean {
